...
module.exports = function(gcloudConfig, gcloudStorageBucket) {
  ...
  function uploadToStorage(req, res, next) {
    ...
    for (var i in req.files) {
      console.log('start to loop files');
      var arrayLenght = parseInt(req.files.length);
      var gcsname = Date.now() + req.files[i].originalname;
      var file = bucket.file(gcsname);
      var stream = file.createWriteStream();
      req.files[i].cloudStorageObject = gcsname;
      req.files[i].cloudStoragePublicUrl = getPublicUrl(gcsname);
      req.files[i].cloudStorageUri = getStorageUri(gcsname);
      ...
      stream.end(req.files[i].buffer, function(){
